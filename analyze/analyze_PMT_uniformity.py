...
if __name__ == "__main__":
    ...
    if not os.path.exists('TTS.txt'):
        with open('TTS.txt','a') as TTS:
            TTS.write('X Y stddev mean filename CFD_threshold_fraction')
    ...
    for fichero in ficheros:
        ...
        plt.title('Time of arrival distribution, CFD at {}% fraction'.format(cfd_frac*100))
        plt.text(0.05, 0.95,'std: {:.4f} ns'.format(fracTs_jitter), transform=ax.transAxes)
        plt.text(0.05, 0.90,'mean: {:.4f} ns'.format(fracTs_mean), transform=ax.transAxes)
        plt.text(0.05, 0.85,'X = '+str(X)+ 'mm ; '+'Y = '+str(Y)+ 'mm', transform=ax.transAxes)
        plt.xlabel(' Time    [ns] ')
        plt.ylabel('Counts')
        name='TOAdistribution_CFD_'+str(cfd_frac)+'_X'+str(X)+'_Y'+str(Y)+'.png'
        ...
        with open('TTS.txt','a') as TTS:
            TTS.write('\n'+str(X)+' '+str(Y)+ ' '+str(fracTs_jitter)+' '+str(fracTs_mean)+' '+str(fichero)+' '+str(cfd_frac)+' '+str(meanAmplitude))
        fd.close()
